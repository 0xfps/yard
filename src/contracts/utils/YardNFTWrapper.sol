...
contract YardNFTWrapper is IYardNFTWrapper, ERC721, Ownable2Step {
    ...
    modifier onlyAllowedPairs() {
        if (!allowedPairs[msg.sender]) 
            revert("YARD: ONLY_ALLOWEDPAIRS");
        _;
    }

    modifier onlyReleased(uint256 id) {
        if (!nftReleasedStatus[id]) 
            revert("YARD: ONLY_RELEASED");
        _;
    }
    ...
    function setFactory(address _factory) public onlyOwner {
        if(_factory == address(0))
            revert("YARD: INVALID ADDRESS");
        factory = _factory;
    }

    function addPair(address _pair) onlyFactory public {
        if(_pair == address(0))
            revert("YARD: INVALID ADDRESS");
    ...
    function transferFrom(address from, address to, uint256 id) public override onlyReleased(id) {
        transferFrom(from, to, id);
    }

    function safeTransferFrom(address from, address to, uint256 id) public override onlyReleased(id) {
    ...
    function wrap(IERC721 nft, uint256 id, address to) public returns (uint256) {
    ...
    function unwrap(uint256 id) public returns (bool) {
        _burn(id);
